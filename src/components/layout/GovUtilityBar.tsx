...
export function GovUtilityBar() {
  ...
  return (
    <div className="gov-utility-bar">
      <div className="max-w-[1440px] mx-auto px-3 sm:px-4 md:px-8 h-8 sm:h-9 flex items-center justify-between gap-2 sm:gap-4">
        {/* Left: India flag + govt label */}
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          <span className="text-sm sm:text-base" role="img" aria-label="India flag">🇮🇳</span>
        ...
        <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
          {/* Font size */}
          <div className="flex items-center gap-0.5 sm:gap-1" aria-label="Adjust font size">
            {labels.map((label, i) => (
              <button
                key={label}
                onClick={() => apply(i)}
                className={`px-1.5 sm:px-2 py-0.5 rounded text-white/80 transition-colors text-[9px] sm:text-[10px] font-semibold ${
                  sizeIdx === i
          ...
          <div className="flex items-center gap-1">
            <span className="material-symbols-outlined text-white/60 text-xs sm:text-sm">translate</span>
            <select
              id="util-lang-select"
              aria-label="Select language"
              value={lang}
              onChange={(e) => setLanguage(e.target.value as SupportedLocale)}
              className="bg-transparent text-white/85 text-[11px] sm:text-xs font-medium focus:outline-none cursor-pointer max-w-[80px] sm:max-w-none"
            >
